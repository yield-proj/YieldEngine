...
public class Project implements Serializable {
    ...
    public boolean updatePropsToLatest(boolean ignoreCheck) throws IOException {
        ...
        if (!new File(path, "Libraries/yieldengine-core.jar").exists()) {
            if (ignoreCheck)
                Files.copy(Objects.requireNonNull(Project.class.getResourceAsStream("/engine/yield-core.jar")), new File(path, "Libraries/yieldengine-core.jar").toPath(), StandardCopyOption.REPLACE_EXISTING);
            else return true;
        }
        if (!new File(path, "Libraries/yieldengine-editor-runtime.jar").exists()) {
            if (ignoreCheck)
                Files.copy(Objects.requireNonNull(Project.class.getResourceAsStream("/engine/yield-editor-runtime.jar")), new File(path, "Libraries/yieldengine-editor-runtime.jar").toPath(), StandardCopyOption.REPLACE_EXISTING);
            else return true;
        }
        if (!new File(path, "Libraries/yieldengine-openalimpl.jar").exists()) {
            if (ignoreCheck)
                Files.copy(Objects.requireNonNull(Project.class.getResourceAsStream("/engine/yield-openalimpl.jar")), new File(path, "Libraries/yieldengine-openalimpl.jar").toPath(), StandardCopyOption.REPLACE_EXISTING);
            else return true;
        }

        if (!new File(path, "Libraries/yieldengine-openglimpl.jar").exists()) {
            if (ignoreCheck)
                Files.copy(Objects.requireNonNull(Project.class.getResourceAsStream("/engine/yield-openglimpl.jar")), new File(path, "Libraries/yieldengine-openglimpl.jar").toPath(), StandardCopyOption.REPLACE_EXISTING);
            else return true;
        }
