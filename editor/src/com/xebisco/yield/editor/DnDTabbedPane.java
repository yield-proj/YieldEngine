 ...
 * <a href="http://java-swing-tips.blogspot.com/2008/04/drag-and-drop-tabs-in-jtabbedpane.html">...</a>
...
public class DnDTabbedPane extends JTabbedPane {
    ...
    private void initTargetLeftRightLine(int next, TabTransferData a_data) {
        ...
        if ((a_data.getTabbedPane() == this)
            ...
            m_lineRect.setRoundRect(rect.x + rect.width - LINEWIDTH / 2., rect.y,
                    s_glassPane.getGhostWidth(), rect.height, arc, arc);
            m_isDrawRect = true;
        } else {
    ...
    private void initTargetTopBottomLine(int next, TabTransferData a_data) {
        ...
        if ((a_data.getTabbedPane() == this)
            ...
            m_lineRect.setRoundRect(rect.x, rect.y + rect.height - LINEWIDTH / 2.,
                    rect.width, LINEWIDTH, arc, arc);
            m_isDrawRect = true;
        } else if (next == 0) {
            Rectangle rect = getBoundsAt(0);
            m_lineRect.setRoundRect(rect.x, -LINEWIDTH / 2., rect.width, LINEWIDTH, arc, arc);
            m_isDrawRect = true;
        } else {
            Rectangle rect = getBoundsAt(next - 1);
            m_lineRect.setRoundRect(rect.x, rect.y + rect.height - LINEWIDTH / 2.,
                    rect.width - 2, LINEWIDTH - 2, arc, arc);
            m_isDrawRect = true;
        }
    }
...
class GhostGlassPane extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        ...
        //if (p.x > getLocationOnScreen().x && p.x < getLocationOnScreen().x && p.y > getLocationOnScreen().y && p.y < getLocationOnScreen().y + getHeight())
        if (m_draggingGhost != null && p != null)
            g2.drawImage(m_draggingGhost, (int) p.getX() - m_draggingGhost.getWidth() / 2, (int) p.getY() - m_draggingGhost.getHeight() / 2, null);
        /*else
